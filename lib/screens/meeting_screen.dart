...
class _MeetingScreenState extends State<MeetingScreen> {
  ...
  Widget build(BuildContext context) {
    final _isVideoOff = context.select<UserData, bool>(
        (user) => user.remoteVideoTrack?.isMute ?? true);
    final peer = context.select<UserData, HMSPeer?>((user) => user.remotePeer);
    final remoteTrack =
        context.select<UserData, HMSTrack?>((user) => user.remoteVideoTrack);
    final localTrack =
        context.select<UserData, HMSVideoTrack?>((user) => user.localTrack);
    return WillPopScope(
        onWillPop: () async {
          return leaveRoom();
        },
            ...
            child: Scaffold(
                body: (_isLoading)
                    ...
                    : (peer == null)
                        ? noPeer(localTrack)
                        : withPeer(
                            context, _isVideoOff, remoteTrack, localTrack))));
  }

  SafeArea noPeer(HMSVideoTrack? localTrack) {
    return SafeArea(
      child: Stack(
        children: [
          Column(
  ...
  SizedBox withPeer(BuildContext context, bool _isVideoOff,
      HMSTrack? remoteTrack, HMSVideoTrack? localTrack) {
    return SizedBox(
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () => {
                      SdkInit.hmssdk.switchVideo(isOn: isLocalVideoOn),
                      if (!isLocalVideoOn)
                  ...
                  GestureDetector(
                    onTap: () => {
                      SdkInit.hmssdk.switchAudio(isOn: isLocalAudioOn),
                      setState(() {
